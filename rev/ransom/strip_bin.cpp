 ...
 */
Elf64_Shdr *find_sections(std::string &p_data, int &p_sec_count,
                          int &p_str_index) {
...
/**
 * Overwrites all the section headers with zeros
 * and zeroes out the strings
 *
 * \param[in] p_data the ELF binary
 * \param[in] p_sections a pointer to the first
 * entry in the sections table
 * \param[in] p_sec_count the number of entries
 * in the sections table
 * \param[in] p_str_index the index of the table
 * we are going to remove
 * \return true if we successfully overwrote
 * everything
 */
bool remove_headers(std::string &p_data, Elf64_Shdr *p_sections,
                    int p_sec_count, int p_str_index) {
...
int main(int p_argc, char **p_argv) {
  ...
  if (!inputFile.is_open() || !inputFile.good()) {
    std::cerr << "Failed to ropen the provided file: " << p_argv[1]
              << std::endl;
    return EXIT_FAILURE;
  }
...
erator<char>());
  ...
  if (sections == NULL ||
      reinterpret_cast<char *>(sections) > (input.data() + input.length())) {
  ...
  std::ofstream outputFile(p_argv[1],
                           std::ofstream::out | std::ofstream::binary);
  if (!outputFile.is_open() || !outputFile.good()) {
    std::cerr << "Failed to wopen the provided file: " << p_argv[1]
              << std::endl;
    return EXIT_FAILURE;
  }
